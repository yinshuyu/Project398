...
//load file - only finds out the size of the dictionary 
bool LoadFile(std::string filename, unsigned& size)
{
	...
	for (i = 0; std::getline(f, line); ++i); //find out the size of the dictionary 
											//- how many passwords are there in the dictionary

	size = i;  //size - number of password in the dictionary
