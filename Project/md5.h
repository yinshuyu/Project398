...
const unsigned int MD5_STRING_SIZE = 2 * MD5_SIZE + 1;      /* 33 */
 ...
 namespace md5 {
    ...
    //extern "C" void sig_to_string(const void* signature_, char* str_, const int str_len);
    ...
    class md5_t {
        public:
            ...
            __host__ __device__  void sig_to_string(const void* signature_, char* str_, const int str_len) {
                unsigned char* sig_p;
                char* str_p;
                char* max_p;
                unsigned int high, low;

                char* HEX_STRING = "0123456789abcdef";    /* to convert to hex */
            ...
            __host__ __device__ md5_t::md5_t() {
                initialise();
            }

            __host__ __device__ md5_t::md5_t(const void* input, const unsigned int input_length, void* signature = NULL) {
            ...
            __host__ __device__ void md5_t::process(const void* input, const unsigned int input_length) {
                if (!finished) {
            ...
            __host__ __device__ void md5_t::finish(void* signature_ = NULL) {
                if (!finished) {
            ...
            __host__ __device__ void md5_t::get_sig(void* signature_) {
                if (finished) {
            ...
            __host__ __device__ void md5_t::get_string(void* str_) {
                if (finished) {
            ...
            __host__ __device__ void md5_t::initialise() {
                /*
            ...
            __host__ __device__  void md5_t::process_block(const unsigned char* block) {
            ...
            __host__ __device__ void md5_t::get_result(void* result) {
